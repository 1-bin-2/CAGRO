...
from open_r1.cagro import parse_strict_cagro_response
...
# Stage 1 and stage 2 must render the same structural contract.
...
class LazySupervisedDataset(Dataset):
    ...
    def _make_conversation_image_and_video(self, example, use_audio_in_video=False):
        ...
        if parse_strict_cagro_response(example["solution"]) is None:
            raise ValueError(
                "Cold-start target does not satisfy the exact CAGRO three-span protocol"
            )
...
def collate_fn(examples):
    ...
    # Cold-start SFT supervises only the assistant completion. Training on the
    # system/user prompt would mismatch stage-2 GRPO, where those tokens are
    # conditioning context rather than policy actions.
    assistant_prefixes = [
        processor.tokenizer.encode(marker, add_special_tokens=False)
        for marker in ("<|im_start|>assistant\n", "<|im_start|>assistant")
    ]
    assistant_prefixes = [prefix for prefix in assistant_prefixes if prefix]
    for row_index, row_ids in enumerate(batch["input_ids"].tolist()):
        assistant_start = None
        for prefix in assistant_prefixes:
            for start in range(0, len(row_ids) - len(prefix) + 1):
                if row_ids[start : start + len(prefix)] == prefix:
                    candidate = start + len(prefix)
                    if assistant_start is None or candidate > assistant_start:
                        assistant_start = candidate
        if assistant_start is None:
            raise ValueError(
                "Could not locate the assistant turn in the rendered chat template; "
